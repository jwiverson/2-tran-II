#Create Ree(3) and find a Schur cover
G:=Ree(3);;
...
pi:=CompositionMapping(hom,inv);;

#Find the stabilizer of a point in a doubly transitive action
#It is the unique maximal subgroup of index q^3+1=28
max:=ConjugacyClassesMaximalSubgroups(G);;
class:=First(max, c-> Size(G)/Size( Representative(c) ) = 28);;
G0:=Representative(class);;

#Find the pre-image of G0 in the Schur covering
...
RadRouxParams:=function(Gast,G0ast,alpha)
	...
	local n,r,x,G0ast2,proj1,proj2,dcstAction,pair,xi,eta,k,z,Cr,er,GastTil,G0astTil,em1,em2,p1,p2,gensG0ast,gensH,zeta,dict,w,j,dcst,count,el,c;
...
#The result shows that Ree(3) only acts doubly transitively on real lines